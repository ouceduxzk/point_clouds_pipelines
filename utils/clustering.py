...
from ransac_plane import RansacPlane
...
def ground_segmentation(data):
    ...
    options = {
        ...
        "max_iter": 20,
        "min_iter" : 2,
        "max_error_distance" : 0.2
    }

    ransac = RansacPlane(options, data)
    ransac.run()
    inliers = ransac.get_inliers()

    colors = np.zeros((data.shape[0], 3))
    colors[inliers, 2] = 255
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(data)
    pcd.colors = o3d.utility.Vector3dVector(colors)
    o3d.io.write_point_cloud("groud_blue.ply", pcd)
    indices = set(range(data.shape[0]))
    segmented_indices = list(indices.difference(inliers))
    segmented_cloud = data[segmented_indices, :]
...
def clustering(data):
    ...
    alg = cluster.KMeans(n_clusters=6)
    alg.fit(data)
    clusters_index = alg.labels_
    # 屏蔽结束
