...
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    if len(point_indices) > leaf_size:
        ...
        # 1. naive implementation o(nlognlogn)
        # point_indices_sorted, _ = sort_key_by_vale(point_indices, db[point_indices, axis])  # M

        # 作业1
        # 屏蔽开始
        # middle_left_idx = math.ceil(point_indices.shape[0] / 2 ) - 1
        # middle_right_idx = middle_left_idx + 1
        # dim = db.shape[1]
        # left_indices = point_indices_sorted[:middle_right_idx]
        # right_indices = point_indices_sorted[middle_right_idx:]

        # middle_left_value = db[point_indices_sorted[middle_left_idx], axis]
        # middle_right_value = db[point_indices_sorted[middle_right_idx], axis]

        # root.value = (middle_left_value + middle_right_value) * 0.5
        # root.left = kdtree_recursive_build(root.left,  db, left_indices,  axis_round_robin(axis, dim), leaf_size)
        # root.right= kdtree_recursive_build(root.right, db, right_indices, axis_round_robin(axis, dim), leaf_size)

          # 2. using median
        median = np.median(db[point_indices, axis])
        left_indices = np.where(db[point_indices,axis]  < median)[0]
        right_indices = np.where(db[point_indices, axis]>  median)[0]
        dim = db.shape[1]
        root.value = median
        root.left = kdtree_recursive_build(root.left, db, left_indices, axis_round_robin(axis, dim), leaf_size)
        root.right= kdtree_recursive_build(root.right, db, right_indices, axis_round_robin(axis, dim), leaf_size)
