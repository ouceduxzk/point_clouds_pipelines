...
def octree_recursive_build(root, db, center, extent, point_indices, leaf_size, min_extent):
    ...
    if len(point_indices) <= leaf_size or extent <= min_extent:
        ...
        # determin which of those points belongs to each 8 subspaces.
        #  00 | 01
        #  10 | 11 in 2d
        for idx in point_indices :
            ...
            children_point_indices[subspace].append(idx)
            # some subspace might has 0 indices
        # determine children center and their extent
        factors = [-0.5 , 0.5]
        for i in range(8):
            child_center_x = center[0] + factors[ i & 1 > 0] * extent
            child_center_y = center[1] + factors[ i & 2 > 0] * extent
            child_center_z = center[2] + factors[ i & 4 > 0] * extent
...
def octree_knn_search(root: Octant, db: np.ndarray, result_set: KNNResultSet, query: np.ndarray):
    ...
    # 作业7
    # 屏蔽开始
    # decide and search the most relevant child
    subspace = 0
    if root.center[0] < query[0]:
        subspace = subspace | 1
    if root.center[1] < query[1]:
        subspace = subspace | 2;
    if root.center[2] < query[2]:
        subspace = subspace | 4;

    if octree_knn_search(root.children[subspace], db, result_set, query):
        return True

    # in case the most relevant children is not true
    for i , child in enumerate(root.children):
        if i == subspace or child is None:
            continue
        if not overlaps(query, result_set.worstDist(), child):
            continue
